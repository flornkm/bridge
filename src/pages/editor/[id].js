      ...
      <div className={"w-screen h-screen bg-neutral-50 bg-cover bg-center " + (project.type === "cookieBanner" && "bg-[url('/images/editor/canvas.svg')]")}>
        <div className="w-full bg-white py-6 fixed top-0 border-b border-b-neutral-200 z-10">
          <div className="max-w-[80%] w-full mx-auto justify-between flex items-center">
            ...
            <Link target="_blank" href={"/" + project.id + "&" + project.name.toLowerCase().replace(/ /g, "-")} className="px-4 truncate py-2 text-gray-600 hover:text-black bg-white ring-1 ring-neutral-200 rounded-xl flex gap-2 items-center max-2xl:absolute max-2xl:top-24 max-2xl:left-[50%] max-2xl:translate-x-[-50%] max-w-screen">
              <Icon.Link size={20} />
              {"bridge.supply/" + project.id + "&" + project.name.toLowerCase().replace(/ /g, "-")}
            </Link>
